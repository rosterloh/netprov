...
| Non-Linux     | `btleplug` | `crates/sdk/src/ble/backend_btleplug.rs`  |
...
User-facing documentation lives in `docs/src/`, with navigation in
`docs/src/SUMMARY.md` and mdBook configuration in `docs/book.toml`.

Every commit must keep the documentation consistent with the behavior,
configuration, commands, or workflows it changes. Update affected pages in the
same commit, add or remove `SUMMARY.md` entries when pages change, and update
`README.md` or `CHANGELOG.md` when their summaries or release notes are affected.
Run `mdbook build docs` before pushing documentation changes.
